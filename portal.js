...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // ─── DOCUMENT DOWNLOADS (FILLABLE PDF) ───
    async function handleFileDownload(btn) {
        ...
        try {
            ...
            if (file) {
                ...
                await addDoc(collection(db, "activity"), {
                    ...
                    body: `You generated a signed copy of: ${file.name}`,
            ...
            alert("Error generating the fillable PDF. Please contact clinical support.");
        } finally {
    ...
    // Listen for all download triggers
    document.addEventListener('click', (e) => {
        const btn = e.target.closest('.download-trigger');
        if (btn) handleFileDownload(btn);
        
        // Special case for dashboard button (triggers first file: Referral)
        if (e.target.closest('#viewSignedPacketDashboard')) {
            handleFileDownload({ dataset: { fileIndex: 0 }, innerHTML: e.target.closest('#viewSignedPacketDashboard').innerHTML, disabled: false });
        }
    });
    ...
    document.getElementById('messageForm')?.addEventListener('submit', async (e) => {
        ...
        await addDoc(collection(db, "messages"), { ...data, clientId: auth.currentUser.uid, senderName: `${cachedUserData.firstName} ${cachedUserData.lastName}`, senderRole: "Client", timestamp: serverTimestamp(), read: true });
    ...
    function initCareTeam(uid) {
        onSnapshot(query(collection(db, "care_teams"), where("clientId", "==", uid)), (snap) => {
            ...
            teamList.innerHTML = ''; snap.forEach(doc => { doc.data().members.forEach(m => {
    ...
    function initMessaging(uid) {
        onSnapshot(query(collection(db, "messages"), where("clientId", "==", uid), orderBy("timestamp", "desc")), (snap) => {
            ...
            msgList.innerHTML = ''; snap.forEach(docSnap => {
                const m = docSnap.data(); const div = document.createElement('div'); div.className = `message-item ${m.read ? '' : 'unread'}`;
    ...
    function initNotifications(uid) {
        onSnapshot(query(collection(db, "notifications"), where("userId", "==", uid), orderBy("timestamp", "desc"), limit(10)), (snap) => {
            ...
            let unread = 0; if (snap.empty) { list.innerHTML = '<p>No alerts</p>'; dot.style.display = 'none'; return; }
            list.innerHTML = ''; snap.forEach(docSnap => { const n = docSnap.data(); if (!n.read) unread++;
            const div = document.createElement('div'); div.className = `notif-item ${n.read ? '' : 'unread'}`;
    ...
    function initActivityLogs(uid) {
        onSnapshot(query(collection(db, "activity"), where("userId", "==", uid), orderBy("timestamp", "desc"), limit(8)), (snap) => {
            ...
            feed.innerHTML = ''; snap.forEach(docSnap => {
                const a = docSnap.data(); const div = document.createElement('div'); div.className = 'activity-item';
    ...
    function initVisitLog(uid) {
        onSnapshot(query(collection(db, "visits"), where("clientId", "==", uid), orderBy("date", "desc")), (snap) => {
            ...
            table.innerHTML = ''; snap.forEach(docSnap => {
                const v = docSnap.data(); const tr = document.createElement('tr');
    ...
    function initTodaySchedule(uid) {
        onSnapshot(query(collection(db, "visits"), where("clientId", "==", uid), where("date", "==", new Date().toLocaleDateString('en-CA'))), (snap) => {
            ...
            area.innerHTML = ''; snap.forEach(doc => {
                const v = doc.data(); const item = document.createElement('div'); item.className = 'schedule-item';
